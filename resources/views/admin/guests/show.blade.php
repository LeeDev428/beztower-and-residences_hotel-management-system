            ...
            <form method="POST" action="{{ route('admin.guests.update', $guestRouteKey ?? ($guest->id ?? $guest->guest_id)) }}">
                ...
                <div style="display: flex; flex-direction: column; gap: 1rem;">
                    ...
                    <div>
                        ...
                        <input type="email" name="email" value="{{ $guest->email }}" style="width: 100%; padding: 0.75rem; border: 1px solid var(--border-gray); border-radius: 8px;" {{ optional(auth()->user())->role === 'manager' ? 'readonly' : 'required' }}>
                    </div>
                    <div>
                        <label style="display: block; margin-bottom: 0.5rem; font-weight: 600;">Phone</label>
                        <input type="text" name="phone" value="{{ $guest->phone }}" style="width: 100%; padding: 0.75rem; border: 1px solid var(--border-gray); border-radius: 8px;" {{ optional(auth()->user())->role === 'manager' ? 'readonly' : 'required' }}>
                    </div>
                    <div>

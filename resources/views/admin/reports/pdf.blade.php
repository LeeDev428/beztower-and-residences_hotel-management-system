...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .header-meta-cell { display: table-cell; width: 220px; vertical-align: top; text-align: right; }
        ...
        .header .generated-by-box { display: inline-block; margin-top: 2px; padding: 6px 10px; border: 1px solid rgba(212, 175, 55, 0.55); border-radius: 6px; font-size: 10px; color: #f2f2f2; }
...
<body>
    ...
    <div class="header">
        <div class="header-wrap">
            ...
            <div class="header-text-cell">
                ...
                <div class="subtitle">Hotel Management System &mdash; Report</div>
                <div class="period">{{ \Carbon\Carbon::parse($startDate)->format('M d, Y') }} to {{ \Carbon\Carbon::parse($endDate)->format('M d, Y') }}</div>
                <div class="generated">Generated: {{ \Carbon\Carbon::now()->format('F d, Y \a\t h:i A') }}</div>
            </div>
            <div class="header-meta-cell">
                <div class="generated-by-box">Generated by {{ $generatedByDisplay ?? ('System (' . ($generatedBy ?? 'System') . ')') }}</div>
            </div>
        </div>
    </div>
            ...
            <td style="width: 49%; vertical-align: top; padding-left: 10px;">
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{ $rt->name }}</td>
                            <td style="text-align: right; font-weight: 600;">PHP {{ number_format($rt->revenue, 2) }}</td>
                        </tr>
                        @empty
                        ...
                        <tr style="border-top: 2px solid #d4af37;">
                            ...
                            <td style="text-align: right; font-weight: 700; color: #d4af37;">PHP {{ number_format($revenueByType->sum('revenue'), 2) }}</td>
                        </tr>
                        @endif
                    </tbody>
                </table>
    ...
    <div class="section">
        ...
        <table>
            ...
            <tbody>
                @forelse($recentBookingRows as $row)
                <tr>
                    <td>{{ $row['index'] }}</td>
                    <td style="font-weight: 600; color: #d4af37;">{{ $row['reference'] }}</td>
                    <td>{{ $row['guest_name'] }}</td>
                    <td>{{ $row['room_label'] }}</td>
                    <td>{{ $row['check_in'] }}</td>
                    <td>{{ $row['check_out'] }}</td>
                    <td style="text-align: center;">{{ $row['nights'] }}</td>
                    <td style="text-align: right; font-weight: 600;">PHP {{ number_format($row['amount'], 2) }}</td>

...
<x-admin.card title="Room Information">
    <form method="POST" action="{{ route('admin.rooms.store') }}" enctype="multipart/form-data">
        ...
        <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 1.5rem; margin-bottom: 1.5rem;">
            ...
            <div>
                ...
                <div style="display: flex; gap: 0.5rem;">
                    <select name="room_type_id" id="roomTypeSelect" required style="flex: 1; padding: 0.75rem; border: 1px solid var(--border-gray); border-radius: 8px;">
            ...
            <div>
                ...
                <input type="number" name="floor" value="{{ old('floor') }}" required min="2" max="8" step="1" style="width: 100%; padding: 0.75rem; border: 1px solid var(--border-gray); border-radius: 8px;">
                <small style="color: #666; font-size: 0.875rem;">Allowed floors: 2, 3, 4, 5, and 8 only.</small>
                @error('floor')
                <div style="color: var(--danger); font-size: 0.875rem; margin-top: 0.25rem;">{{ $message }}</div>
                @enderror
            </div>
        ...
        <div style="margin-bottom: 1.5rem;">
            ...
            <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 0.75rem; padding: 1rem; background: var(--light-gray); border-radius: 8px;" id="amenitiesContainer">
                ...
                <label style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer;">
                    <input type="checkbox" name="amenities[]" value="{{ $amenity->id }}" {{ in_array($amenity->id, old('amenities', [])) ? 'checked' : '' }} style="cursor: pointer;">
        ...
        <div style="margin-bottom: 1.5rem;">
            ...
            <textarea name="description" rows="4" style="width: 100%; padding: 0.75rem; border: 1px solid var(--border-gray); border-radius: 8px;">{{ old('description') }}</textarea>
            @error('description')
            <div style="color: var(--danger); font-size: 0.875rem; margin-top: 0.25rem;">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div style="display: flex; gap: 1rem;">
            <x-admin.button type="primary">Create Room</x-admin.button>

...
<body>
        ...
        <div class="room-preview-card" role="dialog" aria-modal="true" aria-labelledby="roomPreviewTitle">
            ...
            <div class="room-preview-content">
                ...
                <div class="room-preview-inclusions-wrap">
                    <div class="room-preview-inclusions-title">Inclusion:</div>
                    <ul id="roomPreviewInclusions" class="room-preview-inclusions"></ul>
                </div>
    ...
    <script>
        ...
        function openRoomPreviewModal(trigger) {
            ...
            const roomName = trigger.getAttribute('data-room-name') || 'Room';
        ...
        document.addEventListener('click', (event) => {
            const trigger = event.target.closest('[data-open-room-modal]');
            if (trigger) {
    ...
    <div id="homeToastContainer" style="position:fixed;top:1.25rem;right:1.25rem;z-index:99999;display:flex;flex-direction:column;gap:0.6rem;pointer-events:none;max-width:380px;"></div>
    <script>
    (function(){
        ...
        @if(session('success'))
        showHomeToast('{{ addslashes(session('success')) }}','success');
        @endif
        @if(session('warning'))
        showHomeToast('{{ addslashes(session('warning')) }}','warning');
        @endif
        @if($errors->any())
            @foreach($errors->all() as $error)
        showHomeToast('{{ addslashes($error) }}','error');
            @endforeach
        @endif
    })();
    </script>
